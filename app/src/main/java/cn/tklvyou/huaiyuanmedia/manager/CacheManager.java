package cn.tklvyou.huaiyuanmedia.manager;
...
public class CacheManager {
	...
	public static CacheManager getInstance() {
		if (instance == null) {
			synchronized (CacheManager.class) {
				if (instance == null) {
					instance = new CacheManager(MyApplication.getAppContext());
				}
			}
		}
		return instance;
	...
	public <T> String getListPath(Class<T> clazz) {
		...
		return !StringUtils.isEmpty(classPath) ? classPath + KEY_LIST : null;
	}
	/**
	 * @param clazz
	 * @return
	...
	public <T> String getGroupPath(Class<T> clazz) {
		String classPath = getClassPath(clazz);
		return StringUtils.isEmpty(classPath) ? null : classPath + KEY_GROUP;
	}

	private SharedPreferences getSharedPreferences(String path) {
		return StringUtils.isEmpty(path) ? null : context.getSharedPreferences(path, Context.MODE_PRIVATE);
	}
	...
	public <T> List<T> getAllList(Class<T> clazz, String group) {
		return !StringUtils.isEmpty(group) ? getList(clazz, group, -1, 0) : null;
	}
	/**获取列表
	 * @param clazz
	...
	public <T> List<String> getIdList(Class<T> clazz, String group) {
		...
		return sp == null ? null : JSON.parseArray(sp.getString(group, null), String.class);
	}
	...
	public <T> void save(Class<T> clazz, T data, String id, String group) {
		if (data == null || StringUtils.isEmpty(id)) {
